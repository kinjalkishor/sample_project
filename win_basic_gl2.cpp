...
public:
	...
	uint m_win_bg_color = RGB(25, 25, 112);	//RGB(255, 165, 0);
...
private:
	...
	{		
		...
		{
			print("Cannot Register WinApi Class.\n");
			return false;
		}
...
public:
	...
	bool set_fullscreen_mode(int width, int height, int color_bits)
	{
		...
		{
			print("Could not switch to fullscreen.\n");
			return false;
		}
	...
	bool create_window(const char* title, int posx, int posy, int width, int height, bool is_fullscreen, bool is_borderless)
	{
		...
		{
			print("Another Window is already fullscreen.\n");
		}
		...
		if (!is_borderless)
		{
			if (is_fullscreen)
			{            
				if (!set_fullscreen_mode(width, height, k_color_bits))
				{
					print("Could not create fullscreen window with given resolution ({}, {}). Switching to windowed mode.\n", width, height);
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow)
{
	...
	if (!result_ai) { q_msg_boxw_ok(L"WinApp::init Failed"); }
    bool result_cw = nw_app.create_window("Nex Wars", 10, 50, 800, 450, false, false);
	//bool result_cw = nw_app.create_window(0, 0, 1920, 1080, true, false);
    if (!result_ai) { q_msg_boxw_ok(L"WinApp::create_window Failed"); }
